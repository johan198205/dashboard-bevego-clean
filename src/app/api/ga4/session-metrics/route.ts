...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get session metrics: averageSessionDuration and eventsPerSession
    ...
    const row = metricsResp?.rows?.[0];
    const avgSessionDuration = Number(row?.metricValues?.[0]?.value || 0);
    const eventsPerSession = Number(row?.metricValues?.[1]?.value || 0);

    // Timeseries (per day) for averageSessionDuration
    ...
    return Response.json({
      ...
      timeseries,
      notes: ['Källa: GA4 API – averageSessionDuration, eventsPerSession'],
    });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('GA4 session_metrics error:', err || 'Unknown error');
