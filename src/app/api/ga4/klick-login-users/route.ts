...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Timeseries (per day) - users who triggered klick_login event
        ...
        andGroup: {
          expressions: [
            ...
            },
            {
              filter: {
                fieldName: 'eventName',
                stringFilter: { matchType: 'EXACT', value: 'klick_login' }
              }
            }
          ]
        }
    ...
    return Response.json({
      metric: 'klick_login_users',
      start,
      end,
      total,
      timeseries,
      notes: ['Källa: GA4 API – activeUsers filtrerat på event_name="klick_login"'],
    });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('GA4 klick_login_users error:', err || 'Unknown error');
